...
# Argument parsing
parser = argparse.ArgumentParser(description='Resets Migrations/DB and optionally creates debug superuser.')
parser.add_argument('-a', '--admin', action='store_true', help='Create debug superuser.')
...
subprocess.run([sys.executable, 'manage.py', 'makemigrations'], cwd=PROJECT_ROOT, check=True)
subprocess.run([sys.executable, 'manage.py', 'migrate'], cwd=PROJECT_ROOT, check=True)
...
if args.admin:
    ...
    User = get_user_model()
    User.objects.create_superuser(admin_username, 'admin@admin.admin', admin_password)
